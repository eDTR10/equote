...
import { fetchEntityById, updateEntity } from '../../services/entityService';
import { Entity, CustomField } from '../../types/entity';
...
const EditEntity = () => {
    ...
    useEffect(() => {
        const loadEntity = async () => {
            ...
            try {
                ...
                // Set basic entity info
                setEntityName(entity.name);
                setEntityType(entity.entity_type);
                setEntityEmail(entity.email);
                setEntityPhone(entity.mobile_number);

                // Transform API custom fields to our form format
                const formattedFields = entity.custom_fields.map((field: CustomField) => ({
                    id: field.id,
                    name: field.name,
                    fieldType: field.field_type,
                    isRequired: field.is_required,
                    fieldValues: field.field_values.map(val => ({
                        id: val.id,
                        value: val.value
                    }))
                }));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await updateEntity(Number(id), entityData as any);
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <div className="bg-card dark:bg-card p-6 rounded-lg shadow-sm border border-border">
                <h2 className="text-xl font-medium mb-6 text-foreground">Edit Entity</h2>
                    ...
                    <div className="space-y-6">
                        ...
                        {customFields.length === 0 ? (
                            <div className="text-muted-foreground text-center py-4">
                                No custom fields. Click the button above to add a field.
                            </div>
                        ) : (
